                              ...
                              PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_imu = v_imu.begin( ); it_imu < (v_imu.end( ) - 1); it_imu++)
  {
    ...
    // IMU前向传播，每次传播的时间间隔为dt; next state = last state*state transition matrix + noise*noise transition matrix
  ...
  } // endfor: have accumulated all imu
  ...
  for (auto it_kp = IMUpose.end( ) - 1; it_kp != IMUpose.begin( ); it_kp--)
  {
    ...
    for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) // 时间除以1000单位化为s
    {
      ...
      因为旋转矩阵是正交矩阵，所以它的逆矩阵等于它的转置。如果矩阵是实数的，共轭转置就是简单的转置。
    ...
    } // endfor: have process all points between current two imu

  } // endfor: have traversed all imu
}
...
{
  double t1, t2, t3;
  /* 在OpenMP编程中使用的函数，它用于获取从某一初始点开始的墙钟时间（wall-clock time），以秒为单位 */
  t1 = omp_get_wtime( );
