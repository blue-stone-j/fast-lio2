...
// 初始化节点。此处的根节点是指传入的节点，不是整个kd树的节点
...
int KD_TREE::size( )
{
  ...
  else
  {
    ...
    不论哪种类型的锁，都不可能被两个不同的线程同时得到，而必须等待解锁。对于普通锁和适应锁类型，解锁者可以是同进程内任何线程；
    而检错锁则必须由加锁者解锁才有效，否则返回EPERM；对于嵌套锁，文档和实现要求必须由加锁者解锁，但实验结果表明并没有这种限制，
    这个不同目前还没有得到解释。在同一进程中的线程，如果加锁后没有解锁，则任何其他线程都无法再获得锁。*/
...
void KD_TREE::multi_thread_rebuild( )
{
  ...
  while (!terminated)
  {
    // The second thread will lock all incremental updates (i.e., points insert, re-insert, and delete)
    // but not queries on this sub-tree
...
// K近邻搜索
void KD_TREE::Nearest_Search(PointType point, int k_nearest, // input
                             PointVector &Nearest_Points, vector<float> &Point_Distance, double max_dist) // output
{
...
// 前序遍历的点级更新
// 在ik-d树上的点级更新以递归的方式实现，这类似于替罪羊k-d树。对于点集插入，该算法递归地从根节点向下搜索，
// 并将新点的除法轴上的坐标与存储在树节点上的点进行比较，直到发现一个叶节点然后追加一个新的树节点。
// 对于删除或重新插入一个点P，该算法会查找存储该点P的树节点，并修改deleted属性。
...
void KD_TREE::Search(KD_TREE_NODE *root, int k_nearest, PointType point, MANUAL_HEAP &q, double max_dist)
{
  ...
  int retval;
  // The function Pushdown is applied before searching the sub-tree rooted at node T to pass down its lazy labels.
  ...
  } // end: heap is full or there are left nodes
  else
  {
    // update heap
    // left node
...
// 当属性Pushdown 为true时，Pushdown 函数将把deleted, treedeleted,和 pushdown的标签复制到它的子节点（但没有复制到它的的后代。
...
// Update函数的主要目的是为了更新该节点以及它子树的点的范围以及size，还有他的del，bal，用来判断树是否平衡，是否需要重建。
// 这个函数再原论文中是pull-up操作（树的属性由子向父传播）。

// Pullup函数将基于T的子树的信息汇总给T的以下属性：treesize（见数据结构1，第5行）保存子树上所有节点数，
// invalidnum 存储子树上标记为“删除”的节点数，range（见数据结构1，第7行）汇总树上所有点坐标轴的范围，其中k为点维度。
...
// 调整后，新节点如果比新的父节点小，则已经调整到位，如果比新的父节点大，则需要和父节点重新进行交换。至此，最大堆调整完成。
