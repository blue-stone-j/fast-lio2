...
struct KD_TREE_NODE
{
  ...
  float alpha_del; // value to judge whether need delete
  float alpha_bal; // value to judge whether need balance
};

// a func to compare points
...
class KD_TREE
{
 private:
  ...
  void start_thread( );
  void stop_thread( );
  void run_operation(KD_TREE_NODE **root, Operation_Logger_Type operation); // delete and add points
  ...
  float delete_criterion_param   = 0.5f; // 公式18,用于多线程重建树的临时变量
  float balance_criterion_param  = 0.7f; // 公式18，用于多线程重建树的临时变量
